...
- **Skill 加载**:`skill.py`
- **提示词**:`PROMPT.py`(按角色切片拼装:共 5 个片段 + 子 agent/下线确认两份专用提示词,§6.7)
...
├── PROMPT.py            # 提示词片段(_IDENTITY_MAIN/_WORKFLOW/_DELEGATE/_SCHEDULE/_TEAM_LIFE)
│                        #   + build_system_prompt(role) + SUBAGENT_PROMPT / OFFLINE_CONFIRM_PROMPT
...
├── README.md            # 面向第一次看到这个仓库的人:是什么 / 怎么跑 / 代码分布
...
| `PROMPT.py` | 提示词**按角色切片**:5 个具名片段 + `build_system_prompt(role)` 拼装,外加子 agent(`SUBAGENT_PROMPT`,当 user 消息发)与**下线确认回合**(`OFFLINE_CONFIRM_PROMPT`)两份专用提示词(中文) |
...
2. **发送**`claude.py:206`——把 `system_prompt` + 完整 `history` 发给模型。`system_prompt` 由 `PROMPT.build_system_prompt(role)`(按角色取片段,§6.7)+ 长期记忆 + **当前时间** + skill 目录清单拼接而成(`claude.py:115-118`)。发送若因 prompt 过长失败,会走一次「压缩后重试」(`claude.py:207-216`)。**这一步也是超时生效的地方**(§6.2)。
...
设计思想:**把「上下文成本」分层,先用便宜的手段扛住大部分增长,只有逼近上限才动用昂贵的模型总结。** 每层都把被压缩的内容落盘到本地文件,信息不丢失,模型仍可按需读取(系统提示词里专门教了模型如何对待这些归档文件,见 `PROMPT.py:47-65` 的 `_WORKFLOW`;**三个身份共享这一段** —— 子 agent 和成员同样会拿到被截断的工具结果)。
...
### 6.7 `PROMPT.py` — 系统提示词(2026-09-20 重构为按角色切片)

**不再是几个整段常量,而是「具名片段 + 一个拼装函数」。** 片段只有一份,三个身份**引用同一个字符串** ——「改了主 agent 忘了改子 agent」在结构上不可能发生。

| 片段 | 行 | 谁看 | 讲什么 |
|---|---|---|---|
| `_IDENTITY_MAIN` | `:28` | 主 | 你是谁 |
| `_WORKFLOW` | `:37` | **三个身份共享** | 任务规划 + Context Management |
| `_DELEGATE` | `:70` | 主 | SubAgent 使用规则 / 结果处理 / Agent Teams |
| `_SCHEDULE` | `:153` | 主 | 定时任务(只留判断、跨工具机制、护栏) |
| `_TEAM_LIFE` | `:185` | 成员 | 通信规则、反客套、下线握手、长期记忆 |
| `_ROLE_PARTS` | `:213` | — | 角色 → 片段,**「谁看到哪段」的唯一出口** |
| `build_system_prompt(role)` | `:220` | — | 拼装。未知角色**报错**,不给一份空的让它裸奔 |
| `SUBAGENT_PROMPT` | `:237` | 子 | 子 agent 的执行规范(仍是 **user 消息前缀**,不是 system) |
| `OFFLINE_CONFIRM_PROMPT` | `:259` | 成员 | 下线确认的最后一个回合 |

**为什么是切片,而不是「一段大提示词 + 各角色的纠偏附录」**:附录式等于「所有身份先拿主 agent 那一份,再补一句『上面那些不适用于你』」。模型读到的是完整的主 agent 说明,补的那句压不住它 —— `9b37de0` 实测过:成员照着「怎么组建团队」去做,还谎报「已启动成员」。切片式从根上不可能发生:**成员看不到那段**。

这不是新设计,是**把 `9b37de0` 已经立好、`claude.py:118` 已经写成架构的规矩做完**。那条注释说「以前『给哪段提示词』和『给哪些工具』是两套并行机制、会互相漂移,现在合成一套」——但当时只合了工具那半(`tool_filter(role)` + `ROLE_DENIED`,数据驱动),提示词仍是「所有人先拿主 agent 那份,成员再追加一段纠偏」。于是同一个 role 的两半又开始各写各的。缺口的**实测量级**:重构前,成员的 system 是 4465 字符,其中 **85% 在讲主 agent 该怎么派活**;子 agent 连纠偏段都没有。

**顺带删掉了 32 句与工具描述重复的接口细节**(cron 怎么写、`team_list` 的状态枚举、`team_stop` 的语义……)。重复的**方向**很关键:是「工具描述里已经写全了,提示词又抄了一遍」,所以**删提示词那一份不丢信息**,不需要动 `TOOLS.py`。逐句对照(diff 出来的 40 句、每句现在在哪)在 `v1.1.2` 那次提交里,不在这里重复。

**两个自检**(`:288` / `:311`):
- `mentioned_tools(text, universe)`:从提示词里挑出提到了哪些工具名。子串扫描 + **手判词边界**,不引正则 —— 不判边界的话 `bash` 会在 `bash_exec` 里命中。而且一律按**小写原名**匹配:`SubAgent` 这种大写是散文称呼,当命中会误报一大片。
- `check_prompts(role, text, owned, universe)`:该角色看到的提示词里点名的工具,**必须真的在它手里**。方向是**单边的** —— 提到了 → 必须拥有;**反方向不报**(拥有但没提不算错,`task_get` 从没被提过,它靠工具描述自我说明)。和 `check_roles` 同构:收「已知集合」、返回 `list[str]`、**绝不抛异常**(提示词写错不该让整个项目起不来)。

> 提示词与工具描述是**两份配套的说明书**:`PROMPT.py` 讲「什么时候该用哪类工具」「什么时候**别**用」「结果从哪回流」,`TOOLS.py` 里每个工具的 `description` 讲「这个工具怎么用、边界在哪、参数什么格式」。**能在别处表达的,不进提示词** —— 这是判据。留在提示词里的只有四类:**判断**、**跨工具的机制**、**护栏**、**报文格式**。
>
> ★ 但有一条反直觉的:**提示词里有一部分是对抗模型默认倾向的**。删了它不会报错、不会崩,表现只是「模型开始做我原来不希望它做的事」,而且没人会想到是提示词少了一句。**判据是:删之前先问「删了之后模型会不会做出我原来不希望它做的事」,会 → 不删。** 这些句子被 `_a2_spike/probe_prompt.py` 逐条钉死(删一条探针立刻红)。

**护栏清单**——`git log` 挖出来的、花过实测代价的句子。这一轮**一条都没动**,将来也不许:

| 句子 | 来历 |
|---|---|
| 长期记忆由主 agent 统一维护:需要记住什么,说给主 agent,你自己不写(成员 + 子 agent 各一条) | `9b37de0` **实测踩出来的**:成员要写记忆、手里没有 `write_memory`,于是自己用 bash 调 `memory.py` 的 handler 绕过 —— 它把这当成「技术障碍」找了个技术办法。注释原话:「不告诉它为什么,它只会继续绕」 |
| 不要谎报结果 | 测试报告·问题 6:成员谎报「已启动成员」 |
| 不要发没有信息量的消息 | 成员之间互相刷「喵~」,靠这条压住(§6.12「客套风暴」) |
| 拒绝下线并说明理由 | 下线握手必须由成员自己判断,不能无条件退出 |
| 不要直接相信 subagent 声明完成 | 结果处理段的全部价值所在 |
| 不要谎称看到了执行过程 | `b0c0e79` 加的,防幻觉 |
| content 里别写需要用户确认的动作 | `b0c0e79` 加的:定时任务跑在后台、没人在终端 |
| 也不要用 bash sleep 去等 | 反浪费:发消息是异步的,等只会烧 token |
| 不要滥用 subagent | 简单任务别派活 |

> 这些句子**没有一条能靠 `TOOLS.py` 表达** —— 工具描述说的是「怎么调」,它们说的是「**别**用默认倾向去做那件事」。
...
| `PROMPT.py:153-175`(`_SCHEDULE`) | 「定时任务」使用规范:创建 / 查结果 / 管理三段,**仅主 agent 可见**。**明确写了「执行结果不回到这里」**——因为派发器不进用户对话 |
...
1. **客套风暴**。最初 `run_forever` 给成员的消息包装里写的是「如需回复对方,调用 `send_message`」—— 那是在**邀请**回复。实测后果:共识达成后成员之间又刷了 **53 条**消息(38 条成员↔成员),其中 42 条短于 25 字,最后十条是「喵~ 🐱」来回发,靠某次调用碰巧没调工具才停下。改法是把默认动作反过来:**「只有你确实有新的实质信息要告诉对方、或对方问了需要你回答的问题时才回;只是收到/同意/感谢这类客套话不要回 —— 你回一句它再回一句,会没完没了」**。同一段话在成员那份提示词里又写了一遍(当时是 `TEAM_MEMBER_PROMPT`,2026-09-20 起是 `_TEAM_LIFE`,§6.7),并且加了一句「事情谈完了、你无话可说时**就不要再回** —— 停在这里不是失礼,是让对方也能停下来」。
2. **谎报身份与结果**。成员看到的是主 agent 的系统提示词,它会以为自己能建团队(其实它连这个工具都没有),甚至**谎报「已启动成员」**。修法当时是给成员追加一份 `TEAM_MEMBER_PROMPT`;2026-09-20 重构成**按角色切片**后,成员**根本看不到**那套「怎么组建团队」的说明,纠偏段也就不需要了 —— 那些规则现在直接写在 `_TEAM_LIFE` 里(§6.7)。
...
21. **把「防漂移」做成可执行的检查,而不是靠人记得**(2026-09-20)——「两处真相会互相打架」是这个项目反复踩的坑,之前的修法都是「这次记得改两处」。这一轮把它变成了三层各管一段的机制:**L0** 是产品代码里的启动自检(`claude.py:124-129`,三个角色各跑一次 `check_prompts`),零成本、每次启动都跑;**L1** 是确定性探针(`_a2_spike/probe_prompt.py`:护栏清单、角色隔离、共享片段、反向、长度棘轮),把「哪些句子是花过代价换来的、不许删」这个**判断**固化成了断言;**L2** 是真模型行为回归(`_a2_spike/l2_prompt_ab.py`,8 条用例 × 3 次,改前改后各跑一遍)。三层的能力边界要划清:**L0 + L1 只能证明「没退步」,证明不了「提升」** —— 它们验的是拼装对不对,验不出「模型还照不照做」。反过来说,`9b37de0` 那个洞(成员以为自己能建团队)本来是可以被 L0 在启动那一秒就拦住的:**检查有效性的证明,就是它对着今天的代码跑会红** —— 重构前 `check_prompts` 对成员报了 8 个它没有的工具、对子 agent 报了 10 个。
...
### 9.2 提示词改动的 A/B:三层验证,以及为什么 L2 不能省

提示词改动和代码改动不一样:**它不会让任何东西报错**。改坏了的表现是「模型开始做我原来不希望它做的事」,而这在代码评审里一个字都看不出来。所以 2026-09-20 那次重构配了三层验证。

| 层 | 在哪 | 验什么 | 成本 |
|---|---|---|---|
| **L0** | `claude.py:124-129`(产品代码) | 三个角色各自的提示词点名的工具,必须在它自己的工具表里 | 零,每次启动都跑 |
| **L1** | `_a2_spike/probe_prompt.py` | 护栏清单一条不少、角色隔离、共享片段真的共享、删掉的不许回来、长度棘轮 | 秒级,不联网 |
| **L2** | `_a2_spike/l2_prompt_ab.py` | **模型还照不照做** | 8 条 × 3 次 × 2 个版本 ≈ 10 分钟 |

**★ L2 的三条设计约束**(照 §9.1 那套方法做的,但有两处刻意的不同):

1. **不驱动 `main.py` REPL,直接建那个角色的 agent**。REPL 那条路要先让主 agent「去建一个成员」,设置阶段的抖动会盖掉被测行为本身 —— 主 agent 可能压根没建成员,那这条用例测的就是别的东西了。直接 `ClaudeMini(role=…)` 喂一句话 = **只留一个变量**。
2. **工具层打桩**:`ClaudeMini._excute_tool` 被换成「记下调用 + 返回预置结果」,绝不真的执行 —— 不写文件、不起后台、不写记忆。于是单条用例几秒跑完,且唯一的随机性来自模型的选择,正是要测的东西。
3. **每条重复跑 3 次记通过率**,**人工判定、不让模型当裁判**(用模型评模型是循环论证)。8 条用例每条都对应一个**花过代价的已知失败模式**,不是凭空想的(来历写在 `CASES` 的 `why` 字段里)。

**判据是「没有一条从通过变失败」,不是「全过」** —— 基线本来就有做不到的条目,拿「全过」当门槛只会逼着人去改用例。

**基线怎么拿**:改动还没提交,所以 `git stash` 就能回到改前的提示词。★ 这是**这次**才成立的便利,不是通用做法 —— 计划里写的是「必须在改之前先跑一遍」,那个时序约束在「改动已经落盘」的前提下确实可以被 stash 替代,但**得先把两个文件备份到 `%TEMP%` 再 stash**,否则一次 `stash pop` 失败就把工作弄丢了。

**实测结果**(2026-09-20,8 条 × 3 次 × 2 版本):

| | main | team_member | subagent |
|---|---|---|---|
| system 长度 改前 → 改后 | 3801 → **2736**(−28%) | 4465 → **1144**(−74%) | 3801 → **532**(−86%) |

逐条:7 条持平、**1 条改善**(case 4「问定时任务上次跑了吗」2/3 → 3/3,改前那次是**谎称看到了执行过程**,正是 `b0c0e79` 那条护栏要防的)、**0 条退步**。

★ 两条**如实记下、没有当成收益**的观察:
- case 4 的改善 **n=3,不足以称显著**。它说明的是「没退步」,不是「护栏加强了」——这些护栏一个字都没改,本来就不该指望它变好。
- case 6「明确要求并行查三件事」在两边**都是 2/3**,而且失败的那次是同一个失败模式(起了 3 个子 agent 但全是前台 → 串行阻塞,没有真的并行)。这**不是这次改动造成的**,是一个**两边都有的真缺陷**:`is_background` 的建议只写在 `subagent` 的工具描述里,而那次模型判断「这三个任务很快、我要立刻拿到结果」就走了前台。要修得在提示词或工具描述里补一句「用户明确要求并行时用后台」,属于另一个议题。
...
> 仓库历史里有一次**全量 SHA 改写**(2026-09-20):推 GitHub 时被 `GH007` 挡住,原因是提交邮箱 `20244081121@stu.usc.edu.cn` 在那个账号里被设成了私有邮箱。用 `git filter-branch --env-filter` 把 author + committer 换成了 `mjh134@users.noreply.github.com`。**注意 6 个附注 tag 的 tagger 是单独重建的** —— `--tag-name-filter cat` 只改 tag 指向的提交,不改 tag 对象自己的 tagger,漏了这步 tag 照样被拒。改写后 `main^{tree}` 与改前逐字节一致,证明只动了元数据。**因此 `v1.1.2` 之前的任何 SHA 记录都已失效**(包括本文档早期版本里引用的)。
...
4. ~~把 `.env` 加进 `.gitignore`,初始化 git 做版本管理~~ —— **已完成**(2026-09-18,2026-09-20 推到 GitHub `mjh134/claude_mini`):仓库已 `git init`,仓库级 `core.autocrlf=false`,密钥与 `tool_result/` 已挡住。遗留的是**密钥轮换**(§8.3,只有本人能做)和「`core.autocrlf` 这条保护只在本机生效」(§10.3);
5. ~~增加最小可运行测试~~ —— **部分完成**(2026-09-20):提示词这一层已经有三层(§9.2):L0 启动自检(产品代码内)、`_a2_spike/probe_prompt.py`(L1)、`_a2_spike/l2_prompt_ab.py`(L2,真模型 A/B)。**仍然缺的是压缩三层与 cron 解析 / `compute_next_run` 的确定性单测** —— 它们和提示词无关,依旧挂着。顺带记一条边界:L1/L2 两个脚本在 `_a2_spike/`(项目约定:验证脚本不进项目目录),因此**不进 git**;真正的硬不变量已经放在 L0(产品代码里、每次启动都跑),所以探针不进 CI 不致命,要进的话把 `probe_prompt.py` 挪进仓库即可(它只依赖 `PROMPT.py` + `TOOLS.py`,连 agent 实例都不建);
...
14. **提示词:L2 挖出来的那个真缺陷 ——「用户明确要求并行,模型却全用前台子 agent」**。§9.2 的 case 6 在改前改后**都是 2/3**,失败那次是同一个模式:起了 3 个子 agent,但一个 `is_background=true` 都没有 → 它们是**串行**跑的,用户要的「并行」根本没发生。现在的线索只有 `subagent` 工具描述里那句「对于预计耗时较长、或与当前任务无关、不必立刻拿到结果的任务,设置为 true」—— 而模型对「查三件小事」的合理判断是「很快、我要立刻拿到结果」,于是走前台。两条修法:① 在 `_DELEGATE` 里补一句「用户**明确要求并行**时用后台」;② 在 `subagent` 描述里加「需要同时推进多件事时用 `is_background`」。**倾向 ②** —— 它是「怎么用这个工具」,按本项目的判据归工具描述。注意别把 case 6 改成「必须 3/3」:那是拿门槛逼用例,`is_background` 本来就是个判断,不是硬规则;
15. **MCP:补一个「服务器掉线」的显式反馈**。目前子进程若中途死掉,表现是后续 `tools/call` 报错,而**模型看到的是一句工具错误**,它不一定能推断出「那个服务器整个没了」。可参考定时任务的 `next_run` 思路:在 `MCPManager` 里记下每个服务器的存活状态,并在工具错误信息里带上「服务器 X 已退出」。
...
*2026-09-20 这一轮的题目是**提示词到底该负责什么**,起因是三个问题:提示词冗余、它该起什么作用、换了它怎么知道有没有变好。排查后发现问题不在「啰嗦」,而在**同一件事有三份真相** —— 项目早在 `9b37de0` 就立过「权限不进提示词」的规矩,`claude.py` 的注释又把「role 统一决定工具和提示词」写成了架构,但**只做完了工具那一半**:工具是数据驱动的 `tool_filter(role)` + `ROLE_DENIED`,提示词还是「所有人先拿主 agent 那份,成员再追加一段纠偏」。量级是实测的:成员的 system 4465 字符,其中 **85% 在讲主 agent 该怎么派活**;子 agent 连纠偏段都没有。所以这一轮不是引入新设计,是**把项目自己定的架构做完** —— `PROMPT.py` 改成「5 个具名片段 + `build_system_prompt(role)`」,共享片段(`_WORKFLOW`)三个身份**引用同一个字符串**,`TEAM_MEMBER_PROMPT` 消失。结果:main 3801 → 2736、team_member 4465 → **1144**、subagent 3801 → **532**。*

*这一轮真正想留住的是三件事。**第一是判据**:「能在别处表达的,不进提示词」,留下的只有判断、跨工具机制、护栏、报文格式四类;据此删掉 32 句与工具描述重复的接口细节(重复的方向是「工具描述已经写全、提示词又抄一遍」,所以删提示词那一份不丢信息)。**第二是那条反直觉的补充**:提示词里有一部分是**对抗模型默认倾向**的,删了它不会报错、不会崩,表现只是「模型开始做我原来不希望它做的事」,而且没人会想到是提示词少了一句 —— 所以判据要加一问:「删了之后模型会不会做出我原来不希望它做的事?」会 → 不删。第三是**验证**:L0 启动自检 + L1 确定性探针只能证明「没退步」,证明不了「提升」,所以补了一层 L2 真模型行为回归(8 条用例 × 3 次,改前改后各跑一遍,判据是「**没有一条从通过变失败**」而不是「全过」)。结果是 7 条持平、1 条改善(定时任务防幻觉那条 2/3 → 3/3)、**0 条退步**。*

*顺带留下一条方法论:**检查有效性的证明,是它对着今天的代码跑会红**。`check_prompts` 是先写的、先跑的,它对重构前的成员报了 8 个它没有的工具、对子 agent 报了 10 个 —— 那既是检查有效的验证,也是整个重构的理由。反过来,L2 也如实记下了它没修好的东西:case 6「用户明确要求并行,模型却全用前台子 agent」在改前改后**都是 2/3**,是个两边都有的真缺陷,已作为新条目挂进 §11(第 14 条)。*
