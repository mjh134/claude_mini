...
├── 提示词精简清单.md     # 2026-09-20 提示词重构删掉的 40 句各去了哪(§6.7)
...
**顺带删掉了 32 句与工具描述重复的接口细节**(cron 怎么写、`team_list` 的状态枚举、`team_stop` 的语义……)。重复的**方向**很关键:是「工具描述里已经写全了,提示词又抄了一遍」,所以**删提示词那一份不丢信息**,不需要动 `TOOLS.py`。逐条清单见 `提示词精简清单.md`。
...
>
> ★ 但有一条反直觉的:**提示词里有一部分是对抗模型默认倾向的**。删了它不会报错、不会崩,表现只是「模型开始做我原来不希望它做的事」,而且没人会想到是提示词少了一句。`长期记忆由主agent统一维护`(成员 + 子 agent 各一条,`9b37de0` 实测踩出来的)、`不要谎称看到了执行过程`(`b0c0e79`)、`不要发没有信息量的消息`(§6.12「客套风暴」)都是这一类。**判据是:删之前先问「删了之后模型会不会做出我原来不希望它做的事」,会 → 不删。** 这些句子被 `_a2_spike/probe_prompt.py` 逐条钉死。
